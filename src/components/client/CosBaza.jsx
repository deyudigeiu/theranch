...
export default function AdminCosBaza({ ctx }) {
  const { storage, products, cosBaza, setCosBaza, showToast, setAdminPage } = ctx;
  ...
  const save = async () => {
    setSaving(true);
    const updated = { title, description: desc, active, items };
  ...
  const updateQty = (product_id, qty) => {
    if (qty <= 0) {
      ...
      setItems(arr => arr.map(i => i.product_id === product_id ? { ...i, qty } : i));
    }
  };
  ...
  return (
    <div style={{ paddingBottom:100 }}>
      <div style={{ padding:"14px 18px 0", display:"flex", alignItems:"center", gap:12 }}>
        <button onClick={() => setAdminPage("dash")} style={{
          background:"none", border:"none", color:G, fontSize:15, cursor:"pointer", fontWeight:600, padding:0
        }}>‹ Înapoi</button>
        <span style={{ fontWeight:800, fontSize:16, color:"#2D2D2D" }}>Coșul Lunii</span>
      </div>
      ...
      <div style={{ padding:"16px 18px 0" }}>
        ...
        <label style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14, cursor:"pointer" }}>
          <span style={{ fontSize:13, color:"#555" }}>Coș activ (vizibil clienților)</span>
        ...
        <div style={{ position:"relative", marginBottom:12 }}>
          <input value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="Caută produs de adăugat..." style={inp}/>
          {filtered.length > 0 && (
            <div style={{
              ...
              {filtered.map(p => (
                <div key={p.id} onClick={() => addProduct(p)}
                  ...
                  <span style={{ fontSize:18 }}>{p.image?.startsWith('http') ? '🖼️' : p.image||'📦'}</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:600 }}>{p.name}</div>
                    <div style={{ fontSize:11, color:"#777" }}>{p.price} RON / {p.unit}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {items.length === 0 ? (
          <div style={{ textAlign:"center", padding:"24px 0", color:"#aaa", fontSize:13 }}>
            Niciun produs în coș. Caută mai sus pentru a adăuga.
          </div>
        ) : (
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:14 }}>
            {items.map(item => {
              ...
              if (!p) return null;
              return (
                <div key={item.product_id} style={{ ...card, padding:"10px 14px",
                  display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <button onClick={() => updateQty(item.product_id, item.qty - 1)} style={{
                      width:28, height:28, borderRadius:8, border:`1.5px solid ${G}`,
                      background:"white", color:G, fontSize:16, cursor:"pointer", fontWeight:700
                    }}>−</button>
                    ...
                    <button onClick={() => updateQty(item.product_id, item.qty + 1)} style={{
                      ...
                      background:G, color:"white", fontSize:16, cursor:"pointer", fontWeight:700
                    }}>+</button>
                  </div>
                </div>
              );
        ...
        {/* Total */}
        {items.length > 0 && (
          <div style={{ background:GL, borderRadius:12, padding:"12px 16px", marginBottom:20,
            display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontSize:13, color:"#555" }}>Total coș ({items.length} produse)</span>
            <span style={{ fontWeight:800, fontSize:16, color:G }}>{total.toFixed(2)} RON</span>
          </div>
        )}
